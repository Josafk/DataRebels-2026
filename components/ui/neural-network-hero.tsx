...
// ShaderBackground está en NeuralBackground (layout). Sin SplitText (plugin de pago): usamos gsap.from.
...
export default function NeuralNetworkHero({
  ...
  useGSAP(
    () => {
      ...
      gsap.set([headerRef.current, paraRef.current, ctaRef.current, badgeRef.current, ...microItems], {
        opacity: 0,
        y: 20
      });
      ...
      if (headerRef.current) {
        tl.fromTo(headerRef.current, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.9 }, 0.1);
      }
      if (paraRef.current) {
        tl.to(paraRef.current, { opacity: 1, y: 0, duration: 0.5 }, '-=0.55');
      }
      if (ctaRef.current) {
        tl.to(ctaRef.current, { opacity: 1, y: 0, duration: 0.5 }, '-=0.35');
      }
      if (microItems.length > 0) {
        tl.to(microItems, { opacity: 1, y: 0, duration: 0.5, stagger: 0.1 }, '-=0.25');
      }
    },
    { scope: sectionRef }
  );
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen w-full overflow-hidden bg-transparent">
